...
import { completeChapitre, getChapitresCompletes, getFinalTestStatus } from '../../../services/userservice';
...
const CoursDetails = () => {
    ...
    const init = async () => {
      try {
        ...
        if (user?.role === 'etudiant') {
          const [reussisRes, chapitresRes, finalTestRes] = await Promise.all([
            getQuizzesReussis(user._id),
            getChapitresCompletes(user._id),
            getFinalTestStatus(user._id, id),
          ]);

          const ids = reussisRes.data.map((item) =>
            typeof item === 'string' ? item : item.quizId?._id
          );
          setQuizzesReussis(ids);

          const status = {};
          chapitresRes.data.forEach(({ coursId, chapitreId }) => {
            if (coursId === id) status[chapitreId] = true;
          });
          setCompletionStatus(status);

          setFinalTestPassed(finalTestRes.data.passed);
        }
  ...
  const handlegetCertification = async () => {
    try {
      const response = await getCertification(currentUser._id, id);
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `certificate_${cours.nomcours}.pdf`);
      ...
      toast.error('Échec du téléchargement du certificat.');
    }
  };
  ...
  return (
    <div className="cours-detail-layout">
      <aside className="cours-sidebar">
        ...
        <ul>
          ...
          {currentUser?.role === 'etudiant' && (
            <li
              key="final-test"
              className={`final-test-item ${finalTestPassed || (allChaptersCompleted && cours.finalTest) ? '' : 'disabled'}`}
              style={{
                cursor: finalTestPassed ? 'default' : (allChaptersCompleted && cours.finalTest ? 'pointer' : 'not-allowed'),
                opacity: finalTestPassed || (allChaptersCompleted && cours.finalTest) ? 1 : 0.5,
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'space-between',
                padding: '0.5rem 0',
            ...
            >
              {finalTestPassed ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <CheckCircle2Icon style={{ color: 'green' }} titleAccess="Test réussi" />
              ...
              {finalTestPassed && (
                ...
                >
                  ...
                  <span style={{ fontSize: '0.9rem' }}>Certificat</span>
                </button>
              )}
            </li>
          )}
      ...
      <main className="cours-content">
        ...
        {currentUser?.role === 'etudiant' && isLastChapter && !finalTestPassed && (
          <Button
            className="mt-3"
