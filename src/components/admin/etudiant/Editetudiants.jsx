...
const EditetudiantsModal = ({ open, onClose, etudiant, onSave }) => {
  ...
  useEffect(() => {
    if (etudiant && etudiant.userId) {
      const { firstname = '', lastname = '', email = '', tel = '', sexe = '' } = etudiant.userId;
      // Normalize sexe value to match select options
      const normalizedSexe =
        sexe === 'Homme' || sexe === 'M' || sexe === 'male' ? 'Masculin' :
        sexe === 'Femme' || sexe === 'F' || sexe === 'female' ? 'Féminin' :
        sexe || '';
      setFormData({ firstname, lastname, email, tel, sexe: normalizedSexe });
      console.log('etudiant.userId:', etudiant.userId); // Debug log
  ...
  const handleSubmit = () => {
    onSave({ ...etudiant, userId: { ...etudiant.userId, ...formData } });
    onClose();
  };
...
export default EditetudiantsModal;